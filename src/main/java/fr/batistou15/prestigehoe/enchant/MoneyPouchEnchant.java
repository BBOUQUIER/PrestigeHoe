...
public class MoneyPouchEnchant implements HoeEnchant {

    public static final String ID = "money_pouch";
    ...
    public MoneyPouchEnchant(PrestigeHoePlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public void reload(ConfigManager configManager) {
        ...
        if (sec == null) {
            plugin.getLogger().warning("[MoneyPouchEnchant] Section enchants." + ID + " introuvable");
            enabled = false;
            maxLevel = 0;
            defaultLevel = 0;
            baseChance = 0.0D;
        ...
        if (notifSec != null) {
            ...
            notifMessage = notifSec.getString(
                    "on-proc-message",
                    "%prefix%&6Tu as trouvé une bourse d'argent: &a$%amount_formatted%&6 !"
            );
        } else {
            notifEnabled = false;
            notifMessage = null;
        ...
        plugin.getLogger().info(String.format(
                Locale.US,
                "[PrestigeHoe] [MoneyPouchEnchant] enabled=%s, maxLevel=%d, defaultLevel=%d, baseChance=%.4f, chancePerLevel=%.4f, tiers=%d",
    ...
    /**
     * Tente un proc de MoneyPouch à chaque casse de bloc.
     *
     * @return montant d'argent gagné (0.0 si aucun proc)
     */
    public double tryExecute(Player player, HoeData hoe) {
        ...
        EnchantProcHelper.ProcData procData = EnchantProcHelper.computeProcData(
                ...
                profile,
                hoe,
                MoneyPouchEnchant.ID,   // ⚠ bien "money_pouch"
                baseChance,
                chancePerLevel,
                maxChance,
                level
        ...
        // 💰 Donne l'argent au joueur (Vault)
        EconomyHook eco = plugin.getEconomyHook();
        if (eco != null) {
            eco.deposit(player, finalAmount);
        }

        // Stat globale d'argent
        profile.addMoneyEarned(finalAmount);

        // Message de proc si activé + respect des toggles
        if (notifEnabled
                && notifMessage != null && !notifMessage.isEmpty()
                && profile.isEnchantMessagesEnabledGlobal()
                && profile.isEnchantProcMessagesEnabledGlobal()
                && profile.isEnchantProcMessageEnabled(MoneyPouchEnchant.ID)   // ⚠️ bon ID
