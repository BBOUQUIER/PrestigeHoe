...
public class HoeXpBoosterEnchant implements HoeEnchant {

    public static final String ID = "hoe_xp_booster";
    ...
    private String stackMode;       // "ADD" ou "MULTIPLY"
    private double xpBonusPerLevel; // ex: 0.01 = +1% par niveau

    public HoeXpBoosterEnchant(PrestigeHoePlugin plugin) {
        this.plugin = plugin;
    }
    ...
    /**
     * Calcule le multiplicateur d'XP houe provenant de cet enchant.
    ...
    public void reload(ConfigManager configManager) {
        ...
        if (sec == null) {
            plugin.getLogger().warning("[HoeXpBoosterEnchant] Section enchants." + ID + " introuvable");
            enabled = false;
            maxLevel = 0;
            defaultLevel = 0;
            stackMode = "ADD";
        ...
        if (settings != null) {
            ...
            xpBonusPerLevel = settings.getDouble("xp-bonus-per-level", 0.01);
        } else {
            stackMode = "ADD";
            xpBonusPerLevel = 0.0;
        }
        ...
        plugin.getLogger().info(String.format(
                "[PrestigeHoe] [HoeXpBoosterEnchant] enabled=%s, maxLevel=%d, defaultLevel=%d, stackMode=%s, xpBonusPerLevel=%.4f",
